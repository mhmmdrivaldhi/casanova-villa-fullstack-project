...
const UpdateForm = ({amenities, room}: {amenities: Amenities[], room: RoomProps}) => {
        ...
        <form action={formAction}>
            <div className="grid md:grid-cols-12 gap-5">
                <div className="col-span-8 bg-white p-4">
                    <div className="mb-4">
                        <input type="text" name="name" defaultValue={room.name} className="py-2 px-4 rounded-sm border border-gray-400 w-full" placeholder="e.g Summer Luxury Room . . ." />
                        <div aria-live="polite" aria-atomic="true">
                            <span className="text-xs text-red-500 italic mb-2">{state?.error?.name}</span>
                        </div>
                    </div>
                    <div className="mb-4">
                        <textarea name="description" defaultValue={room.description} rows={5} className="py-2 px-4 rounded-sm border border-gray-400 w-full" placeholder="Description . . ." />
                        <div aria-live="polite" aria-atomic="true">
                    ...
                    <div className="mb-4 grid md:grid-cols-3">
                        {amenities.map((item) => (
                            <div className="flex items-center mb-4" key={item.id}>
                                <input type="checkbox" name="amenities" className="w-4 h-4 text-blue-600 bg-gray-100 border border-gray-400 rounded" defaultValue={item.id} defaultChecked={checkedAmenities.includes(item.id)}/>
                ...
                <div className="col-span-4 bg-white p-4">
                    ...
                    <div className="mb-4">
                        <input type="text" name="capacity" defaultValue={room.capacity} className="py-2 px-4 rounded-sm border border-gray-400 w-full" placeholder="e.g 10 Person . . ." />
                        <div aria-live="polite" aria-atomic="true">
                            <span className="text-xs text-red-500 italic mb-2">{state?.error?.capacity}</span>
                        </div>
                    </div>
                    <div className="mb-4">
                        <input type="text" name="price" defaultValue={room.price} className="py-2 px-4 rounded-sm border border-gray-400 w-full" placeholder="e.g Rp. 6.000.000 . . ." />
                        <div aria-live="polite" aria-atomic="true">
